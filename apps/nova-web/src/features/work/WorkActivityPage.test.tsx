...
const runtimeEvents: WorkActivityRuntimeEvent[] = [
  {
    eventId: 'event-001',
...
function activityResponse(
  events: readonly WorkActivityRuntimeEvent[] = runtimeEvents,
...
function installBffFetch({
  ...
  events?: readonly WorkActivityRuntimeEvent[];
} = {}) {
  const fetchMock = vi.fn<typeof fetch>(async (input, init) => {
    expect(String(input)).toBe(workActivityPath('work-001'));
    expect(init?.method).toBe('GET');
    expect(init?.credentials).toBe('include');
    expect(init?.body).toBeUndefined();
    return jsonResponse(activityResponse(events));
  });
...
describe('Work Activity Runtime read-only', () => {
  it('renders Runtime events through one GET-only authenticated BFF capability call', async () => {
    const fetchMock = installBffFetch();
    ...
    expect(resolveRouteLocation(window.location.pathname).pathParams.workId).toBe('work-001');
    expect(fetchMock).toHaveBeenCalledTimes(1);
    expect(fetchMock.mock.calls.map(([input]) => String(input))).toEqual([
      workActivityPath('work-001'),
    ]);
  ...
  it('renders the empty state when the mission has no Runtime events', async () => {
    installBffFetch({ events: [] });
    renderWorkSurface('/work/work-001/activity');

    expect(await screen.findByRole('heading', { name: 'No activity available' })).toBeInTheDocument();
  });

  it('renders the error state after a BFF/Runtime failure without a fixture fallback', async () => {
